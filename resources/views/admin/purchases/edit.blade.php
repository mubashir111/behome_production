...
<div class="max-w-[1200px] mx-auto pb-12">
    ...
    <div class="mb-8 flex items-center justify-between gap-4 flex-wrap">
        <div>
            <h2 class="text-3xl font-bold font-outfit text-slate-900">Edit Purchase #PO-{{ str_pad($purchase->id, 5, '0', STR_PAD_LEFT) }}</h2>
            <p class="text-slate-500 mt-1">Modify supplier purchase and update inventory intake.</p>
        </div>
    ...
    <form id="purchaseForm" action="{{ route('admin.purchases.update', $purchase->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-[2rem] border border-slate-300 shadow-sm p-8">
                    ...
                    <div class="space-y-5">
                        <div class="admin-form-group mb-0">
                            ...
                            <input type="date" name="date" class="admin-input" required value="{{ old('date', \Carbon\Carbon::parse($purchase->date)->format('Y-m-d')) }}">
                        </div>
                        ...
                        <div class="admin-form-group mb-0">
                            <label class="admin-label">Reference No</label>
                            <input type="text" name="reference_no" class="admin-input" value="{{ old('reference_no', $purchase->reference_no) }}">
                        </div>

                        <div class="admin-form-group mb-0">
                            <label class="admin-label">Status *</label>
                            <select name="status" class="admin-select" required>
                                <option value="5" {{ old('status', $purchase->status) == 5 ? 'selected' : '' }}>Pending</option>
                                <option value="10" {{ old('status', $purchase->status) == 10 ? 'selected' : '' }}>Ordered</option>
                                <option value="15" {{ old('status', $purchase->status) == 15 ? 'selected' : '' }}>Received</option>
                            </select>
                ...
                <div class="bg-white rounded-[2rem] border border-slate-300 shadow-sm p-8">
                    ...
                    <div class="space-y-5">
                        <div class="admin-form-group mb-0">
                            ...
                            <textarea name="note" class="admin-textarea" placeholder="Internal memo, delivery references...">{{ old('note', $purchase->note) }}</textarea>
                        </div>
                        <div class="admin-form-group mb-0">
                            <label class="admin-label">Attachment (Invoice PDF/Image)</label>
                            @if($purchase->getFirstMediaUrl('purchase'))
                                <div class="mb-3">
                                    <a href="{{ route('admin.purchases.downloadAttachment', $purchase->id) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 underline">Current Attachment</a>
                                </div>
                            @endif
