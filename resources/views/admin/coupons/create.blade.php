...
<div class="pb-12">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold font-outfit text-slate-900 mb-2">Create New Coupon</h1>
            <p class="text-slate-500 mt-1">Configure a new promotional discount for your customers.</p>
        </div>
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 admin-card">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-5 py-3 rounded-2xl border-2 border-slate-300 focus:border-indigo-600 outline-none transition-all bg-white text-slate-900"
                              placeholder="Brief details about this coupon...">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="discount_type" required
                        ...
                        <option value="5" {{ old('discount_type') == 5 ? 'selected' : '' }}>Fixed Amount ($)</option>
                        <option value="10" {{ old('discount_type') == 10 ? 'selected' : '' }}>Percentage (%)</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Discount Amount</label>
                    <input type="number" step="0.01" name="discount" value="{{ old('discount') }}" required
                           class="w-full px-5 py-3 rounded-2xl border-2 border-slate-300 focus:border-emerald-600 outline-none transition-all bg-white text-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Minimum Order Amount</label>
                    <input type="number" step="0.01" name="minimum_order" value="{{ old('minimum_order', 0) }}" required
                           class="w-full px-5 py-3 rounded-2xl border-2 border-slate-300 focus:border-emerald-600 outline-none transition-all bg-white text-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Maximum Discount Amount</label>
                    <input type="number" step="0.01" name="maximum_discount" value="{{ old('maximum_discount', 0) }}" required
                           class="w-full px-5 py-3 rounded-2xl border-2 border-slate-300 focus:border-emerald-600 outline-none transition-all bg-white text-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Limit Per User (Optional)</label>
                    <input type="number" name="limit_per_user" value="{{ old('limit_per_user', 1) }}"
                           class="w-full px-5 py-3 rounded-2xl border-2 border-slate-300 focus:border-emerald-600 outline-none transition-all bg-white text-slate-900">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-300 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="start_date" value="{{ old('start_date', date('Y-m-d\TH:i')) }}" required
            ...
            <div class="flex flex-col md:flex-row gap-8 items-start">
                <div class="w-full md:w-1/3">
                    <div class="relative group aspect-square rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all hover:border-pink-300">
                        <img id="preview_image" src="{{ asset('images/default/coupon/coupon.png') }}" 
                ...
                <div class="flex-1 space-y-4">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Select Coupon Banner</label>
                    ...
                    <p class="text-xs text-slate-400">Dimensions: 800x800px recommended. Max size: 2MB.</p>
                </div>
            </div>
        </div>
        ...
        <div class="admin-form-actions">
            ...
            <button type="submit" class="admin-btn-primary">Create Coupon</button>
        </div>
    </form>
</div>
