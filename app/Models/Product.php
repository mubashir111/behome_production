...
class Product extends Model implements HasMedia
{
    ...
    {
        if (!empty($this->getFirstMediaUrl('product'))) {
            $mediaItem = $this->getMedia('product')->first();
            $conversionPath = $mediaItem->getPath('thumb');
    ...
    public function getCoverAttribute(): string
    {
        if (!empty($this->getFirstMediaUrl('product'))) {
            $mediaItem = $this->getMedia('product')->first();
            // Check if the conversion file actually exists on disk before using it
            $conversionPath = $mediaItem->getPath('cover');
            if ($conversionPath && file_exists($conversionPath)) {
                return $mediaItem->getUrl('cover');
            }
            // Fallback to original image if conversion doesn't exist
            return $mediaItem->getUrl();
        }
    ...
    public function getPreviewAttribute(): string
    {
        if (!empty($this->getFirstMediaUrl('product'))) {
            $mediaItem = $this->getMedia('product')->first();
            $conversionPath = $mediaItem->getPath('preview');
            if ($conversionPath && file_exists($conversionPath)) {
    ...
    {
        $this->addMediaConversion('thumb')->crop('crop-center', 112, 120)->format('webp')->quality(90)->sharpen(10);
        $this->addMediaConversion('cover')->crop('crop-center', 248, 270)->format('webp')->quality(90)->sharpen(10);
        $this->addMediaConversion('preview')->crop('crop-center', 512, 512)->format('webp')->quality(90)->sharpen(10);
    }
    ...
    {
        return $this->hasMany(Stock::class, 'product_id')->where('status', Status::ACTIVE);
    }
