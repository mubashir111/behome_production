    ...
    public function payment($order, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            if (isset($response['id']) && $response['id'] != null) {
                // Store PayPal order ID so webhook can look up our order if redirect is missed
                DB::table('capture_payment_notifications')->where('order_id', $order->id)->delete();
    ...
    public function success($order, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            if (isset($this->response['status']) && $this->response['status'] == 'COMPLETED') {
                ...
                // Remove capture notification so the webhook doesn't double-process
                DB::table('capture_payment_notifications')->where('order_id', $order->id)->delete();
