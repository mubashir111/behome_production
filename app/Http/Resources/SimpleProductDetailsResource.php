...
class SimpleProductDetailsResource extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        return [
            ...
            'name'                => $this->name,
            'slug'                => $this->slug,
            'price'               => Carbon::now()->between($this->offer_start_date, $this->offer_end_date) ? AppLibrary::convertAmountFormat(max(0, $price - $this->discount)) : AppLibrary::convertAmountFormat($price),
            'currency_price'      => AppLibrary::currencyAmountFormat(Carbon::now()->between($this->offer_start_date, $this->offer_end_date) ? AppLibrary::convertAmountFormat(max(0, $price - $this->discount)) : AppLibrary::convertAmountFormat($price)),
            'old_price'           => AppLibrary::convertAmountFormat($price),
            'old_currency_price'  => AppLibrary::currencyAmountFormat($price),
            'discount'            => Carbon::now()->between($this->offer_start_date, $this->offer_end_date) ? AppLibrary::convertAmountFormat($this->discount) : 0,
            'discount_percentage' => $price > 0 ? AppLibrary::convertAmountFormat(($this->discount / $price) * 100) : 0,
