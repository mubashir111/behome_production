...
class PaymentGatewayService
{
    ...
    {
        try {
            if (isset($validationRequests['payment_gateway_id'], $validationRequests['gateway_status'])) {
                $this->gateway = PaymentGateway::find($validationRequests['payment_gateway_id']);
                if (!blank($this->gateway)) {
                    $this->gateway->status = $validationRequests['gateway_status'];
                    $this->gateway->save();
                }
            }

            if (!blank($validationRequests)) {
                // Skip meta keys that are not GatewayOption fields
                $skipKeys = ['payment_gateway_id', 'payment_type', 'gateway_status'];
